...
namespace Infraestrutura.Repositorio.Genericos
{
    public class RepositorioGenerico<T> : IGenericos<T>, IDisposable where T : class
    {
        ...
        public async Task<List<T>> Listar()
        {
            using (var data = new Contexto(_OptionsBuilder))
            {
                return await data.Set<T>().AsNoTracking().ToListAsync(); // AsNoTracking() evita de trazer toda a config do Entity, basicamente faz uma select no banco e busca oq vc precisa
            }
        }
